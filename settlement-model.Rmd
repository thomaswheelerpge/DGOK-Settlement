...
for (account in missing_accounts) {
  ...
  # Bind the missing data to rt_tidy_data
...
rrh_inflow <- 60 rt_data_filter("INFLOW_MWH-DGLS01") %>% select(Value) %>% sum()

# Distribute RRH Gen in accordance with the following logic
# The maximum amount of total generation (rrh_inflow) should be sold during HL hours 7-22
# The amount sold in an individual hour cannot exceed RRH Max for that hour
# The amount sold during each hour should be equal across the 16 hr HL block
...
df <- df %>%
  mutate(
    ...
    `RRH Gen` = ifelse(Hour >= 7 & Hour <= 22,
                     pmin(min(`RRH Max`), ifelse(rrh_inflow/16 < `RRH Min`, `RRH Min`, rrh_inflow/16)),
                     0),
    rrh_inflow_leftover = rrh_inflow - sum(`RRH Gen`[7:22]),
    `RRH Gen` = ifelse(Hour %in% c(1:6, 23:24),
                     ifelse(Hour %in% c(1:6, 23:24) & rrh_inflow_leftover > 0,
                            rrh_inflow_leftover / sum(Hour %in% c(1:6, 23:24)),
                            0),
                     `RRH Gen`)
  )
...
df <- df %>%
  mutate(
    #
    `RRH Gen` = ifelse(`RRH Gen` < `RRH Min`, `RRH Min`, `RRH Gen`)
  )
